...
minimum pins. A row marked *optional* degrades gracefully when absent.

### Frontend Core

* **[Next.js v16.3.0](https://nextjs.org)** — App Router + Turbopack on Node ≥20.9. Server-side proxy routes are the only path to backend credentials; the browser bundle ships zero secrets.
* **[React v19.2.8](https://react.dev)** — the workspace is one client page with eight role tabs; every subtab is URL-addressable and survives back/forward.
* **[TypeScript v5.9.3](https://www.typescriptlang.org)** — strict mode; contract fixtures generated by the Python engine are type-checked on this side (§12 parity).
* **[Tailwind CSS v4.3.3](https://tailwindcss.com)** — utilities only, bridged onto a hand-written token system in `app/globals.css` that owns both theme palettes and an AA contrast contract enforced by `tests/theme.test.ts`. No preflight — the house reset stays authoritative.
* **[Lucide v1.28.0](https://lucide.dev)** — the only icon dependency. Charts are hand-rolled SVG on one scale kit (`components/chart-kit.tsx`); there is deliberately no chart library.

### Backend

* **[Python v3.12.13](https://www.python.org)** — the gateway runtime inside the container.
* **[FastAPI v0.141.1](https://fastapi.tiangolo.com)** — 34 documented routes; the OpenAPI schema is a committed contract (`tools/openapi.json`) whose SHA-256 the web build verifies at `prebuild`.
* **[Uvicorn v0.52.1](https://www.uvicorn.org)** — **one process, no workers, by design**: the risk gateway holds a mutable in-memory book, a resting-order book, a token bucket and the kill switch; a second worker would fork the book and localise the halt.
* **[Pydantic v2.13.4](https://docs.pydantic.dev)** — every API payload, risk decision and bot read-model shares one schema module (`modules/schemas.py`).
* **[httpx v0.28.1](https://www.python-httpx.org)** / **[websockets v17.0.1](https://websockets.readthedocs.io)** — venue WebSocket ingest (Binance + Bybit L2) and all outbound HTTP, including the Supabase mirror (chosen over `supabase-py` to keep the import graph network-free for CI).
* **[NumPy v2.5.1](https://numpy.org)** / **[pandas v3.0.5](https://pandas.pydata.org)** — the reference backtest engine and all TCA/risk maths. **[vectorbt](https://vectorbt.dev)** is an optional accelerator (`requirements.txt`); the NumPy engine is the documented fallback and `/health` reports which is live.
...
* **[DuckDB v1.5.5](https://duckdb.org)** — the **authoritative** store: an embedded, append-only audit log (orders, events, backtests, equity) on a named Docker volume, with an SQLite fallback. Embedded on purpose: the desk must keep trading when every network dependency is down.
* **[PostgreSQL v17.6](https://www.postgresql.org) on [Supabase](https://supabase.com)** — the durable **mirror**, never a second decision-maker: every gateway decision streams through a bounded queue into `public.order_blotter` with `decided_by` provenance, measured `latency_ms`, and the full 15-gate check vector. RLS deny-by-default (zero `anon` policies), append-only by trigger, `search_path` pinned on every `SECURITY DEFINER` function. Six ordered migrations in [`../supabase/migrations/`](../supabase/migrations/); `tests/test_supabase_schema.py` pins SQL limit defaults to `config.py` offline.
* **[pgvector v0.8.2](https://github.com/pgvector/pgvector)** — 384-dim HNSW cosine index over `public.research_documents` (see RAG & ML below).

### DevOps & Infrastructure

* **[Docker v29.7.2](https://www.docker.com)** — two-stage `python:3.12-slim` image ([`docker/gateway.Dockerfile`](docker/gateway.Dockerfile)), non-root uid 10001, stdlib health probe, compose file at the repo root. `tests/test_container_contract.py` rejects any secret-shaped literal in the committed files.
* **[Caddy v2.6.2](https://caddyserver.com)** — TLS termination on the VM with automatic Let's Encrypt issuance for the sslip.io hostname; the gateway token never crosses the internet in cleartext.
* **[Oracle Cloud](https://www.oracle.com/cloud/)** (Singapore) — the always-on host. Region is load-bearing: US egress gets Binance HTTP 451 / Bybit 403 (§11).
* **[Vercel](https://vercel.com)** (`sin1`) — two serverless projects (web portal, OpenBB service) from one repo with different Root Directories; artifact custody via an Ed25519-signed build attestation against a trust root pinned in reviewed source (`web/lib/artifact-trust.mjs`).
* **[GitHub Actions](https://github.com/features/actions)** — four network-free jobs (gateway, OpenBB, web, repo-audit): a red build means the code broke, never that an exchange was slow. 396 gateway + 689 web + 13 service tests.
* **[Supabase CLI](https://supabase.com/docs/guides/cli)** — migration push via the IPv4 session pooler (the direct DB host is IPv6-only), edge-function deploys.
...
**Retrieval corpus** — what the desk already records, made searchable. Three
document kinds in `public.research_documents`, written through the same
bounded-queue discipline as the mirror: completed **backtest runs** (symbol,
strategy, params, Sharpe, DSR verdict, OOS Sharpe, PBO, `data_hash`), session
**execution summaries**, and **risk incidents**. The backfill tool
(`tools/backfill_research_rag.py`) replays the audit log's existing
`backtest_runs` table — the corpus predates the feature.

**Embeddings** — [`gte-small`](https://huggingface.co/Supabase/gte-small)
(384-dim, unit-normalised) served by `Supabase.ai` inside the
[`embed-research` edge function](../supabase/functions/embed-research/index.ts):
no paid API, no key, no model weights in the gateway image. `body` stores the
exact embedded text so a renderer change can never silently invalidate stored
vectors. An embed outage stores `embedding_status='pending'` — **never a zero
vector**, which is equidistant from everything and would rank as "similar" to
any query.

**Retrieval trigger** — a precisely-defined execution anomaly, not vibes: an
accepted fill whose *realised* slippage exceeds the pre-trade ceiling
(`max_est_slippage_bps`), a rejection citing `est_slippage`/`daily_drawdown`,
or the drawdown breaker engaging. The top-3 similar historical reports attach
to the alert already being broadcast and cache on
`GET /api/research/rag/status`; ad-hoc queries go through
`POST /api/research/rag/search`. When Supabase is absent the routes return a
typed `unavailable` — never `[]`, because "searched, found nothing" is a
different fact.

**Quant/statistical ML in the engine** (no external ML service):
[Deflated Sharpe Ratio](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2460551)
and PSR for multiple-testing correction, PBO (probability of backtest
overfitting), walk-forward out-of-sample validation, Kupiec VaR backtesting
with traffic-light zones, historical + parametric VaR/ES, Kelly sizing,
regime classification, and Monte Carlo resampling bands — Python is the
reference implementation, TypeScript reproduces it against committed fixtures
(§12), so a number on the phone cannot disagree with the screen without a
test failing.
