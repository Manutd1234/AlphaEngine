...
**Source/worktree and release evidence audited: 2026-09-02.** Current topology, locked/runtime
...
| **Quant Developer** | *Can I change this safely?* | Typed contracts, OpenAPI snapshot, a generated TypeScript client (`web/lib/gateway-contract.generated.ts`), parity suites (Python ↔ TypeScript to 1e-4, Python ↔ C++ to the bit), CI, and three suites re-measured 2026-09-02: gateway **3,491 passed / 1 skipped** locally, web **6,840 passed / 6 skipped** across 1,461 suites, service **24 passed**; main CI also runs live services and the real cross-encoder. The separate rendered audit retains its measured **872/872** state/viewport record | No property-based fuzzing; browser qualification remains a separate release activity (`desk-sweep.mjs` and `npm run audit:layout`) rather than the default Node suite |
...
npm test       # 6,840 passed, 6 skipped, across 1,461 suites (2026-09-02)
```
...
| **[Supabase](https://supabase.com)** | managed | Hosts that Postgres. RLS is deny-by-default with explicit grants; the only anonymous table policy is the row-filtered, read-only demo blotter. **Forty-one** ordered migrations are present in [`../supabase/migrations/`](../supabase/migrations/), bundled and were applied/verified live by schema run `33633200876` on 2026-09-02; they cover decision mirrors, account state, research/graph/image, ML lineage, data operations and diffusion. The newest, `20260831131000_research_chunk_replace.sql`, atomically replaces a document's chunk set and removes stale siblings only after every incoming text embedding is ready; otherwise the previous complete generation remains retrievable. `tools/bundle_migrations.py` emits `supabase/apply_all.generated.sql`, and `tests/test_migration_bundle.py` fails when any ordered migration is absent. The manual schema workflow remains the deployment boundary. |
...
../supabase/                Postgres mirror + RAG: 41 worktree/bundled migrations,
...
CI run `33637631574` passed Live services and the eight-case real-weight suite;
schema run `33633200876` applied and verified Oracle/Supabase, and E2E run
...
re-verified on 2026-09-02 by deploy run `33633139022`, schema run `33633200876`
