  ...
  switch (intent.kind) {
    ...
    case "flatten_all":
      return {
        ...
        why: "A closing order for every open position through the same pre-trade gates, one at a time so they cannot race the gateway's exposure accounting.",
        action: "flatten",
        confirmWord: "FLATTEN",
      };
    case "flatten_symbol":
      return {
        ...
        why: "A closing order in the opposite direction, sized to the current notional, through the same gates as an opening order.",
        action: "flatten",
        confirmWord: "FLATTEN",
        symbol: intent.symbol,
      };
    ...
    <div className="handoff-panel" role="dialog" aria-modal="false" aria-labelledby="handoff-title">
      ...
      {sandbox && (
        <div className="banner warn" role="status">
          ...
          <div>
            <strong>This is the sandbox book.</strong> The position does not exist, so execution is
            disabled; the request is shown for its shape only.
          </div>
        </div>
      )}
