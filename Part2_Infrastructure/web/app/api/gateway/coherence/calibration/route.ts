...
import { isCoherenceCalibration } from "@/lib/coherence/types-lab";
...
export async function GET(request: Request) {
  ...
  for (const key of ["horizon_s", "harvest"]) {
    const value = incoming.get(key);
    if (value) forwarded.set(key, value);
  }
  const query = forwarded.toString();
  const result = await callGateway(`/api/coherence/calibration${query ? `?${query}` : ""}`, {
    subject: "how well the settled prices scored",
    validate: isCoherenceCalibration,
    timeoutMs: 25_000,
  });
