...
import { isCoherenceShell } from "@/lib/coherence/types-lab";
...
export async function GET(request: Request) {
  ...
  const result = await callGateway(`/api/coherence/shell${query ? `?${query}` : ""}`, {
    subject: "the watched universe as a filesystem",
    validate: isCoherenceShell,
    timeoutMs: 25_000,
    context,
  });
