 ...
 * It was 588 lines. The two halves that were about a *number* rather than about
 * the loop moved to siblings when it passed 585:
 *
 *   ./overview-latency   the millisecond network figure — tone, chip copy, ring
 *   ./decision-plane     the microsecond in-process figure the gateway pushes
 *
 * They stayed apart on purpose: `DecisionLatencySource` exists so a missing
 * decision figure renders a dash WITH a cause rather than the network number
 * wearing the decision label, and a single module holding both invites exactly
 * that substitution.
 *
 * Both are re-exported below, name by name — `export *` would sweep them up in
 * one line and make a later rename in a sibling silent at the path callers use.
 */

import type { HealthSourceState, SystemHealth } from "@/components/systems/types";
...
export type { LatencyHistoryPoint, LatencyToneKind, LatencyToneResult } from "./overview-latency";
