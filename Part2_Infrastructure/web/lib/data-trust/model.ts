/**
 * The data-trust verdict model: one headline judgement about whether the
 * numbers on the desk can be believed, with the evidence behind it.
 *
 * Split out of `lib/data-trust.ts` when that file passed 780 lines. This half
 * is the derivation the banner and the Trust tab both read; the SLI tiles are
...
export function deriveDataTrust(health: SystemHealth | null, options: DataTrustOptions = {}): DataTrustModel {
  ...
  if (options.healthError) {
    ...
    verdict = {
      ...
      detail: "Stale, degraded or flagged evidence in the observed scope needs an operator decision.",
      tone: "warn",
    };
    ...
    verdict = hasExactProbe && exactContract
      ? {
          ...
          detail: `${options.symbol ?? "The active instrument"} was checked on the exact ${options.probe?.cache.state ?? "served"} payload from ${options.probe?.provenance?.provider ?? "the selected provider"}; the aggregate remains empty.`,
          tone: "good",
        }
      : {
          label: "Not yet proven",
          detail: "Nothing contract-checked in this function instance. Zero evidence is not a clean bill of health.",
          tone: "unknown",
        };
  } else {
    verdict = hasExactProbe && exactContract
  ...
  const actions: DataTrustAction[] = [
    {
      ...
      detail: fatalRisk || flaggedRisk
        ? hasExactProbe
          ? "Reconciliation and the quality ledger diagnose the flagged payload."
    ...
    },
    {
      ...
      detail: exactContract || validation?.evaluated
        ? "Follow provider selection, cache identity, raw and normalised output."
        : "Trace a request through the real registry and contract path.",
