...
import type {
  DataSection, DeveloperSection, ExecutionSection, OverviewSection,
  PortfolioSection, ReliabilitySection, ResearchSection, RiskSection,
} from "@/lib/sections";

/** What a stop needs from the shell: the tab switch, and the eight rail setters. */
...
export function buildTourStops(deps: TourDeps): TourStop[] {
  ...
  return [
    stop("Overview → Decision loop", "The desk in one screen; every pipeline stage links into its tab — research reaches execution only through the risk gate.", "overview", "loop", () => deps.setOverviewSection("loop")),
    ...
    stop("Execution → Trade", "Fire the Fat finger $500k preset — the gate vector names the exact check that refused it, decided in ~0.2 ms.", "live", "trade", () => deps.setExecutionSection("trade")),
    ...
    stop("Data → Incidents", "Simulate a provider outage and watch the incident row, failover graph and consensus react — then self-restore.", "data", "incidents", () => deps.setDataSection("incidents")),
