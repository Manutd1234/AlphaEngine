...
const TAB_FACTS: ReadonlyArray<readonly [string, string]> = [
  ...
  ["REPOSITORY_MANIFEST_PROVENANCE.commit", "the commit in the explorer's inline provenance row"],
  ...
  ["npm run catalog:refresh", "the refresh command in the explorer's inline provenance row"],
  ...
  ["not source contents", "what the Code & Diffs hero note says the manifest withholds"],
  ...
  ["Configured; unverified", "repository comparisons that have configuration but no current-run attestation"],
  ["no live CI attestation for this commit", "why configured schema comparisons are not green verdicts"],
  ...
  ["Local output; unverified", "the package stage outside Vercel"],
