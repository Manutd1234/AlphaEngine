...
// Re-measured 2026-08-24 for the tenth tab. Each is the width at which the
// state above it stops fitting, rounded up to a ten — the CSS comment carries
// the ten figures the sweep read. 1170 is rung 10, which is new: after every
// other rung the row still needs 1171px and the <select> switchover is at 900,
// so there had been nothing left to fold across a 260px band.
const RUNGS = [2050, 1950, 1860, 1660, 1600, 1380, 1290, 1170] as const;
...
describe("the rungs exist, in priority order, and each takes only what it says", () => {
  ...
  it("rung 3 (≤1860): only the Settings label", () => {
    const r = rung(1860);
  ...
  it("rung 4 (≤1800): the data-tier label; rung 5 (<1740): the Connect label", () => {
    assert.match(css, /@media \(max-width: 1800px\) \{\n  \.data-tier__label \{\n    display: none;/);
    assert.match(read("components/header/TelegramCta.tsx"), /max-\[1740px\]:hidden/);
  });
  ...
  it("rung 6 (≤1660): the providers chip to its dot, aria keeps the sentence", () => {
    const r = rung(1660);
  ...
  it("rung 7 (≤1600): brand tagline and tab padding, nothing of the chip", () => {
    const r = rung(1600);
  ...
  it("rung 8 (≤1380): the decision figure folds to its gauge — last of the chip", () => {
    assert.match(rung(1380), /\.latency-chip__copy \{\n    display: none;/);
  });

  it("rung 9 (≤1290): Kill switch and Sign in labels fold to icons that keep their names", () => {
    const r = rung(1290);
  ...
  it("rung 10 (≤1170): the wordmark, leaving the mark that is still the button", () => {
