...
*Source/worktree state and release evidence audited on **4 September 2026**.
...
suites are green. The 2026-09-04 generated record is 3,508 gateway tests,
6,856 web tests across 1,464 suites and 24 service tests; main CI also passed
...
**The live-family completion slice is shipped.** Broad mode discovers up to
200 current open Kalshi event families and hydrates their active markets with
bounded bulk reads. Numeric, categorical and independent surfaces now render
according to the axis the venue actually supplies; Proofs falls back to
per-market executable book constraints when no cross-market relation exists;
and the private RFQ read performs exactly one bounded retry on transient 429/5xx
responses. The latest 120-state UI registry passed all 360 requested
route/viewport combinations with no geometry or console failure.
...
| A quoted family without structural metadata is tested as `book`, not rejected as untestable | absence of a cross-market relation says nothing about whether each executable quote obeys its own probability bounds. Ask ≥ 0, bid ≤ 1 and ask − bid ≥ 0 are still testable per market; only absent quoted inputs justify “untestable” | treating every event as mutually exclusive (invents a sum-to-one relation); returning no test (throws away valid book evidence) | `modules/coherence/kernel/constraints.py`, `modules/coherence/syscalls/certify.py` |
...
| Named and independent outcomes render probabilities but not invented moments | moments require a numeric axis. A named candidate has no meaningful average name, and unrelated YES/NO markets do not become one exhaustive distribution because they share an event response | assigning ordinal indices to labels; normalising unrelated prices to sum to one; showing an empty graph as a failed read | `modules/coherence/kernel/distribution.py`, `web/components/coherence/surface/CategoricalProbabilityBars.tsx` |
...
| The signed RFQ read retries one transient response, once | a single 429/5xx can be transport noise, while an unbounded retry inside an interactive 25-second request can multiply private-channel traffic and hide an outage. One 200 ms retry distinguishes a blip from a persistent venue failure | no retry; an open-ended retry loop; converting persistent 503 into an empty quote list | `modules/coherence/drivers/rfq_reader.py`, `tests/test_coherence_rfq_retry.py` |
