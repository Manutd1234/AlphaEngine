...
the skip count, not the pass count: on 3.12 the suite shows exactly one skip
(`tests/test_data_ops_postgrest.py`, reporting honestly that no Supabase
credentials were in the environment). A second skip means the interpreter is
wrong. Build the venv with `python3.12 -m venv venv`, at
...
| **Neo4j Aura** | A **rebuildable read model** of `research_edges`, projected one-way by [`modules/research_graph_projection.py`](../../Part2_Infrastructure/modules/research_graph_projection.py). Postgres stays authoritative; drift is a non-event — drop the graph and re-project. | An unset `NEO4J_URI` is the normal deployment, reported with a named reason, never an exception. |
...
| [`requirements-dev.txt`](../../Part2_Infrastructure/requirements-dev.txt) | ruff, the native toolchain and communities, on top of core. Separate so a reviewer running tests never installs a linter and the runtime image never carries tooling. | `ruff check .` is unavailable; CI still runs it. |
...
| [`requirements-graph.txt`](../../Part2_Infrastructure/requirements-graph.txt) | The `neo4j` driver for the projection sweep. | Reported the same way as an unset `NEO4J_URI`; Postgres remains authoritative; nothing is on the request path. |
...
- **BGE re-ranker** (`BAAI/bge-reranker-base`) runs locally as ONNX on the CPU
  ...
  must never pay for a download, and CI is network-free by construction.
- **Gemini** (via `google-genai` and `GEMINI_API_KEY`/`GEMINI_MODEL`) writes
  Stage-5 grounded answers behind five fences: the refuse band is checked
  before the call, the context is closed, figures are quoted never computed,
  one fabricated citation refuses the whole answer, and the call is bounded by
  timeout and token cap. `corpus_silent` is a correct verdict, not an error.
...
1,706 passed + 1 skipped, web 3,861 tests across 832 suites, OpenBB service 14.
