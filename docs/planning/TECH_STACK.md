...
| [`requirements-coherence.txt`](../../Part2_Infrastructure/requirements-coherence.txt) | scipy (the coherence LP through HiGHS) and cryptography (RSA-PSS request signing). Neither is on the read path: Kalshi's markets, events, series, orderbooks, trades and fee feeds are all public, which is why the engine runs with no account at all. | Without scipy the closed-form family checks still find Dutch books and every certificate says `engine: closed_form`, so a weaker answer announces itself. Without cryptography the handful of signed endpoints (`/account/limits`, the demo portfolio surface) are unavailable and the status route says so. Adopted into `requirements-dev.txt` because its absence SKIPS four signing tests — the rule that file states — including the only one that signs a real vector and verifies it. |
