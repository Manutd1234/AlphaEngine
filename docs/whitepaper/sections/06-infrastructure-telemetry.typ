...
and the source audit found #measured("42 ordered migrations present in the worktree and generated bundle", "supabase/migrations/*.sql and supabase/apply_all.generated.sql"). The newest RFQ-membership successor provisions every existing and future authenticated account for the fixed paper-only desk while leaving anonymous guests denied. Schema workflow run `33633200876` applied and verified the preceding 41 migrations, including the atomic chunk-replacement RPC, on 2026-09-02. Deployment run `33633139022` restored the full Diffusion ledger, and E2E run `33633746350` passed all 16 production checks, including Neo4j readback. These are dated deployment observations rather than timeless runtime-health claims.
