...
with two editors holding ~90 % of the CPU (load average 3.5), and it shows it:
the *Python reference* moved from 23.0 to 25.6 µs p50 in the same window, and
nothing in this pass touched the Python engine. The p99 column moved further
still. When the whole machine slows, an unpinned in-process benchmark reports
that slowdown as latency, which is why the two changes in this pass were
