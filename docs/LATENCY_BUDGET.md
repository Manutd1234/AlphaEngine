...
<!-- bench:start -->
Regenerated by `tools/bench_decision.py` on 2026-08-19 — 5000 orders after 500 warm-up, 2 venues, arm64 · Python 3.12.14, pinning: unavailable (darwin), median of 7 runs. Exact per-decision samples from `perf_counter_ns`; the live desk reads the 12 %-bucket histogram instead.

| engine | p50 | p99 | p99.9 | max | n |
|---|---|---|---|---|---|
| python — decision | 23.0 µs | 33.2 µs | 53.2 µs | 62.6 µs | 5000 |
| native — decision | 15.2 µs | 24.1 µs | 43.0 µs | 52.6 µs | 5000 |
| native — core (inside the engine) | 83 ns | 125 ns | 208 ns | 291 ns | 5000 |
<!-- bench:end -->
