...
|---|---|---|---|---|---|
| python — decision | 24.9 µs | 35.1 µs | 59.1 µs | 77.6 µs | 5000 |
| native — decision | 13.5 µs | 20.5 µs | 42.8 µs | 69.0 µs | 5000 |
| native — core (inside the engine) | 83 ns | 125 ns | 208 ns | 625 ns | 5000 |

The core row is a tick count, not a nanosecond count: `steady_clock` here advances in 41.667 ns steps (measured by the extension itself) and `duration_cast` truncates, so 83 ns IS two ticks and 125 ns IS three. The distribution behind that row, same samples:

| ticks | ns floor | count | fraction | cumulative |
|---|---|---|---|---|
| 1 | 41 | 1032 | 0.2064 | 0.2064 |
| 2 | 83 | 3901 | 0.7802 | 0.9866 |
| 3 | 124 | 51 | 0.0102 | 0.9968 |
| 4 | 166 | 9 | 0.0018 | 0.9986 |
| 5 | 208 | 5 | 0.0010 | 0.9996 |
| 7 | 291 | 2 | 0.0004 | 1.0000 |

**Fraction inside 2 ticks (83.3 ns): 0.9866** — 4933/5000 calls (per run: 0.9886, 0.9892, 0.9904, 0.9846, 0.9866, 0.9874, 0.9836, 0.9800, 0.9780). That fraction, not the p99 column, is what "p99 under 100 ns" means on this clock.
<!-- bench:end -->
...
happened with another editor's test suite on the same eighteen cores, load
average 3.7 — which is why the *Python reference* reads 24.9 µs p50 here against
23.1 on a quiet machine, with nothing in this pass touching the Python engine.
An earlier regeneration in an earlier pass showed the same thing at 25.6 µs.
...
125 ns p99 are two and three ticks of a ~41.67 ns clock, so that p99 is exactly
one tick above the median — which is why the tick histogram is generated beside
it and is the row to read. **The compiled core** below has what that means for
the desk's sub-100 ns p99 request, and corrects what this document used to
claim about it.
...
p50, 125 ns p99, 208 ns p99.9, 250 ns max** on the run in the table above.
...
**Read all four as tick counts, not as three significant figures — the p99
above all.** `steady_clock` on this machine advances in ~41.67 ns steps, and
every one of the four is a whole number of them, truncated to integer
nanoseconds by `duration_cast`: 83 ns = 2 ticks (83.33), 125 ns = 3, 208 ns = 5
(208.33), 250 ns = 6. Nothing between them is representable. So the p99 sits
**exactly one tick above the median**, and the p50→p99 gap is the resolution of
the clock rather than a property of the code.

**Which is why the desk's requested core p99 of 50–90 ns is not a number this
measurement can produce.** 90 ns falls between the second and third tick, so a
p99 of ≤90 ns would require 99 % of samples to land in the median's *own*
bucket — it would be reporting the timer's floor, not the battery's speed.
Three runs at n = 5 000 put it at 125 ns each time. A smaller figure here would
be a rounding artefact published as a speed-up, which is the single thing this
document exists to prevent. Resolving the p99 any finer needs a finer clock — a
cycle counter, or a batched loop over N iterations reporting an amortised mean,
which answers a different question and hides the tail this document is about —
not faster code.
...
| Arithmetic core (inside the decision) | **83 ns** p50 · 125 ns p99 dev Mac · **320 ns** p50 / 352 ns p99 production VM | C++ battery incl. the routed walk, timed with `steady_clock` inside the engine; the Mac p99 is one ~41.67 ns tick above its p50 — the clock's floor, not the code's, and why ≤90 ns is unmeasurable here (§2); self-measured at startup, bit-exact vs Python |
