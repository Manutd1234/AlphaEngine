...
<!-- bench:start -->
Regenerated by `tools/bench_decision.py` on 2026-08-19 — 5000 orders after 500 warm-up, 2 venues, arm64 · Python 3.12.14, pinning: unavailable (darwin), median of 9 runs. Exact per-decision samples from `perf_counter_ns`; the live desk reads the 12 %-bucket histogram instead.

| engine | p50 | p99 | p99.9 | max | n |
|---|---|---|---|---|---|
| python — decision | 23.1 µs | 33.2 µs | 49.2 µs | 56.5 µs | 5000 |
| native — decision | 12.4 µs | 19.4 µs | 36.8 µs | 44.8 µs | 5000 |
| native — core (inside the engine) | 83 ns | 125 ns | 208 ns | 250 ns | 5000 |
<!-- bench:end -->

**Read that table with its machine in mind.** It was regenerated on 2026-08-19,
unpinned, on a laptop. (`bench_decision.py` stamps the block with the UTC date;
the local clock read 2026-08-20 that evening. The block's date is the one to
quote, because the block is generated and this sentence is not.) An earlier
regeneration in the same pass — two editors holding ~90 % of the CPU, load
average 3.5 — put the *Python reference* at 25.6 µs p50 against 23.0 on a quiet
machine, with the p99 column moved further still, and nothing in that pass
touched the Python engine. When the whole machine slows, an unpinned in-process
benchmark reports that slowdown as latency, which is why the two changes in this
pass were adopted on interleaved A/B evidence rather than on this table:
...
one tick above the median. **The compiled core** below has what that means for
the desk's 50–90 ns p99 request.
