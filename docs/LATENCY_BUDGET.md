...
<!-- bench:start -->
Regenerated by `tools/bench_decision.py` on 2026-08-16 — 5000 orders after 500 warm-up, 2 venues, arm64 · Python 3.12.14, pinning: unavailable (darwin), median of 3 runs. Exact per-decision samples from `perf_counter_ns`; the live desk reads the 12 %-bucket histogram instead.

| engine | p50 | p99 | p99.9 | max | n |
|---|---|---|---|---|---|
| python — decision | 24.8 µs | 27.5 µs | 51.2 µs | 55.1 µs | 5000 |
| native — decision | 28.2 µs | 31.1 µs | 53.2 µs | 59.3 µs | 5000 |
| native — core (inside the engine) | 42 ns | 42 ns | 83 ns | 125 ns | 5000 |
<!-- bench:end -->
...
the arithmetic gates and times *its own work* with `steady_clock`: **42 ns
p50, 42 ns p99** — the arithmetic is at the Apple-Silicon clock-tick floor
(~41.67 ns), which is to say a warm two-venue decision computes in a single
tick. That is a real, measured, nanosecond figure, and it is the answer to
"can the compute be nanoseconds": the compute already is.

It is not, however, a latency win end-to-end. The *whole* decision is ~3 µs
slower on the native engine (28 µs vs 25 µs p50) because crossing the pybind11
boundary and rebuilding the C++ ladders per call costs more than the fold it
replaces, while the Python path reuses its cached sorted views. Both figures
are ~0.004 % of the 68 ms round trip to Binance, so the difference is
invisible to a trader — which is exactly §2.1's original point, now measured
from the other side. The native engine is run so the desk can *show* the
nanosecond core figure the work produced; the header labels the whole
decision (µs) and the core (ns) separately, and the network (ms) is a third
plane that no language touches. Getting there surfaced two silent-wrongness
bugs worth recording: CPython 3.12's `sum()` uses Neumaier compensated
summation, so a plain C++ `+=` fold lands one ULP off and fails the
byte-exact parity fixture (the core reproduces each fold with the matching
algorithm), and FMA contraction fused a multiply-add one ULP off even under
`-ffp-contract=off` until pinned with `#pragma STDC FP_CONTRACT OFF`. The
parity fixture (`web/tests/fixtures/gate-parity.json`) is what caught both.
