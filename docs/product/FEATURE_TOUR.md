...
| **Findings** | Effect plot · Findings table · Instrument | What did the study conclude, and was it fit to? The reported admissible run carries its own 57-meeting out-of-sample score: the baseline predicts some of the clock, while the text adds no qualifying effect. A stage selector plus adjustable t and shuffled-p thresholds recompute the evidence gate locally over the full finding rows; **Instrument** keeps the verdict separated from the mechanism that produced it. |
