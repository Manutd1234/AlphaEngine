...
**The headline numbers, all re-measured 2026-08-22:** 1,719 gateway + 3,883 web + 14 service tests, one
...
| **[GitHub Actions](https://github.com/features/actions)** | managed | Four network-free CI jobs (`gateway`, `openbb-service`, `web`, `repo-audit`) plus a manual `live-smoke`: 1,719 gateway + 3,883 web + 14 service tests. `deploy.yml` ships the gateway (build → GHCR → SSH swap → verify → roll back, and a warning if the container fell back to the Python engine); `openbb-keepalive.yml` pings the research service every ten minutes. |
...
venv/bin/python -m pytest                            # 1,719 passed, 1 skipped
venv/bin/python tools/synthetic_probe.py             # book → cost → risk gate → audit
(cd web && npm test)                                 # 3,883 tests across 838 suites
...
goes stale the week after it is written. The 1,719 needs the native core built
