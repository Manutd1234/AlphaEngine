...
**Last verified: 2026-09-04.** Use [`docs/CURRENT_STATE.md`](docs/CURRENT_STATE.md)
...
2026-09-04 documentation release used Python 3.12.14 with the native core and
the locally configured optional re-ranker; the owning generator recorded:

| Run | Passed | Skipped | Total |
|---|---:|---:|---:|
| 2026-09-04 local count refresh | 3,507 | 1 | 3,508 |
...
2. `tests/test_research_rerank_real.py` — `pytest.skip(..., allow_module_level=True)`,
   ...
   isolated `rerank-real` job does exactly that in a setup step. It runs on
   every `main` push and explicit dispatch, and on pull requests carrying the
   `rerank` label.
...
Refreshed 2026-09-04, it reads gateway **3,508 / 3,507 / 1**, web **6,856
tests across 1,464 suites**, and service **24**. It goes stale
...
fails the push on the mismatch. Refresh it from a tree with only committed
files in it, or the count describes work that is not in the commit.
...
**All four generated artefacts were current on 2026-09-04, and two of them are
...
because this table is what someone reads when a gate fails, and a digest that
does not exist sends them chasing a number they cannot reproduce. Nothing checks
this column, which is exactly why it drifts.

| Artefact | State | Regenerator |
|---|---|---|
| `web/lib/repository-manifest.generated.json` | current — **2,432 paths**, generated 2026-09-04 and accepted by `npm run build`; when behind, `generate-codebase-manifest.mjs --check` reports the added/removed paths | `npm run catalog:refresh` |
| `web/lib/test-counts.generated.ts` | current — web 6,856 across 1,464 suites, gateway 3,508 / 3,507 / 1, service 24, refreshed 2026-09-04; when behind, CI's web-count step fails the push | `npm run counts:refresh` |
...
| `web/lib/gateway-openapi-digest.generated.ts` | current — the checker verified **`fde95f8b…`** against the 76-path / 79-operation `tools/openapi.json` contract on 2026-09-04. Take the value the checker prints: it is a canonical-JSON SHA-256 with sorted keys, not a raw file hash | `python tools/export_openapi.py`, then the digest module, then `node --import tsx scripts/generate-gateway-client.ts` |
...
parameters into an unrelated suite. `RESEARCH_IMAGE_MODEL_PATH` is blanked by
assignment there as well, so a seeded CLIP directory cannot leak into an
unrelated suite.
...
runner imports modules and inspects source/contracts; its 1,464 suites do not
launch Chromium.
